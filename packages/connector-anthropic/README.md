...
```bash
npm install @authensor/connector-anthropic @anthropic-ai/sdk
```
...
1. Before each `messages.create()` call, the request content is evaluated against your policy using the Authensor engine.
...
4. If the policy says **allow**, the request is forwarded to the Anthropic API.
